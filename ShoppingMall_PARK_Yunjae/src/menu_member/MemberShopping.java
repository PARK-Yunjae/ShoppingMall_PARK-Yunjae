...
public class MemberShopping implements MenuCommand{
	...
	public boolean update() {
		...
		cont.setNext("MemberMain"); // 구매가 끝나면 메인으로 오류나도 메인으로
		...
		if(cartNum != -1) { // 있다면
			Cart info = cDAO.getcList().get(cartNum); // 기존 카트를 불러와
			info.setItemCnt(info.getItemCnt()+cnt);   // 개수만 추가
